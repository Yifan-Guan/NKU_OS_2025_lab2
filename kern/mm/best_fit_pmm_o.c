...
best_fit_alloc_pages(size_t n) {
    ...
    if (page != NULL) {
        ...
        if (page->property > n) {
            ...
            // 将剩余部分重新插入空闲链表（保持地址有序）
            list_entry_t* insert_le = &free_list;
            int inserted = 0;
            while ((insert_le = list_next(insert_le)) != &free_list) {
                struct Page *insert_page = le2page(insert_le, page_link);
                if (p < insert_page) {
                    list_add_before(insert_le, &(p->page_link));
                    inserted = 1;
                    break;
                }
            }
